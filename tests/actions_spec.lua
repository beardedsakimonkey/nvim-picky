...
t.describe("actions.edit", function()
  ...
  t.it("opens quoted option help tags", function()
    local ctx = make_ctx({ { tag = "'winborder'" } })
    actions.edit(ctx)
    t.eq(true, ctx.closed)
    t.eq("help", vim.bo.filetype)
    t.ok(vim.api.nvim_get_current_line():find("*'winborder'*", 1, true), "cursor on the option tag's line")
    vim.cmd("helpclose")
  end)
